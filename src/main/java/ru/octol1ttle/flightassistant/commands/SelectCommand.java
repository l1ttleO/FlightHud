...
public class SelectCommand {
    ...
    private static void registerSpeed(LiteralArgumentBuilder<FabricClientCommandSource> select) {
        select.then(literal("speed")
                .then(literal("managed")
                        .executes(context -> {
                            HudDisplayHost.getHost().autoflight.selectedSpeed = null;
                            return 0;
                        })
                )
                .then(argument("target", IntegerArgumentType.integer(0, 30))
                        .executes(context -> {
                            HudDisplayHost.getHost().autoflight.selectedSpeed = IntegerArgumentType.getInteger(context, "target");
                            return 0;
                        })
                )
        );
    ...
    private static void registerAltitude(LiteralArgumentBuilder<FabricClientCommandSource> select) {
        select.then(literal("altitude")
                .then(literal("managed")
                        .executes(context -> {
                            HudDisplayHost.getHost().autoflight.selectedAltitude = null;
                            return 0;
                        })
                )
                .then(argument("target", IntegerArgumentType.integer(-120, 1200))
                        .executes(context -> {
                            HudDisplayHost.getHost().autoflight.selectedAltitude = IntegerArgumentType.getInteger(context, "target");
                            return 0;
                        })
                )
        );
    ...
    private static void registerHeading(LiteralArgumentBuilder<FabricClientCommandSource> select) {
        select.then(literal("heading")
                .then(literal("managed")
                        .executes(context -> {
                            HudDisplayHost.getHost().autoflight.selectedHeading = null;
                            return 0;
                        })
                )
                .then(argument("target", IntegerArgumentType.integer(0, 360))
                        .executes(context -> {
                            HudDisplayHost.getHost().autoflight.selectedHeading = IntegerArgumentType.getInteger(context, "target");
                            return 0;
                        })
                )
        );
