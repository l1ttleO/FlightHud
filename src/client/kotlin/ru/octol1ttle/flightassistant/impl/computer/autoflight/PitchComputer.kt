...
class PitchComputer : Computer(), PitchController {
    ...
    private var currentPitchMode: Text? = null
    ...
    override fun tick(computers: ComputerAccess) {
        ...
        val finalInput: ControlInput? = inputs.filter {
            it.priority.value == inputs[0].priority.value
                    && !limiters.any { limiter -> limiter.blockPitchChange(computers, if (it.target > pitch) Direction.UP else Direction.DOWN) }
                    && it.target >= (minimumPitch?.target ?: -90.0f)
                    && it.target <= (maximumPitch?.target ?: 90.0f)
    ...
    private fun updateSafePitches(computers: ComputerAccess) {
        ...
        minimumPitch = if (minimums.isNotEmpty())
            minimums.filter { it.priority.value == minimums[0].priority.value }.maxBy { it.target }
        else null

        val maximums: List<ControlInput> = limiters.mapNotNull { it.getMaximumPitch(computers) }.sortedBy { it.priority.value }
        maximumPitch = if (maximums.isNotEmpty())
            maximums.filter { it.priority.value == maximums[0].priority.value }.minBy { it.target }
        else null
    }
