...
private fun worldSpaceToScreenSpace(deltaPos: Vec3d, useNoRollMatrix: Boolean = true): Vec3d {
 ...
 */
private fun isScreenSpaceVisible(pos: Vec3d?): Boolean {
    if (pos == null) {
        return false
    }
...
fun getScreenSpaceX(heading: Float): Int? {
    val vec: Vec3d = worldSpaceToScreenSpace(Vec3d.fromPolar(0.0f, heading - 180.0f))
    if (!isScreenSpaceVisible(vec)) {
        return null
    }
...
fun getScreenSpaceY(pitch: Float): Int? {
    val vec: Vec3d = worldSpaceToScreenSpace(Vec3d.fromPolar(-pitch, mc.entityRenderDispatcher.camera.yaw))
    if (!isScreenSpaceVisible(vec)) {
        return null
    }
...
fun getScreenSpace(deltaPos: Vec3d): Vec3d? {
    val vec: Vec3d = worldSpaceToScreenSpace(deltaPos, false)
    if (!isScreenSpaceVisible(vec)) {
        return null
    }
